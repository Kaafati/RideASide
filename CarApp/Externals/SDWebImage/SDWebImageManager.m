...
{
    [self downloadWithURL:url delegate:delegate options:options userInfo:[NSDictionary new]];
}
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didUpdatePartialImage:(UIImage *)image{
    [self imageDownloader:downloader didUpdatePartialImage:image progress:0.f];
}

- (void)imageDownloader:(SDWebImageDownloader *)downloader didUpdatePartialImage:(UIImage *)image progress:(CGFloat)progress
{
    ...
    for (NSInteger idx = (NSInteger)[downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if ([delegate respondsToSelector:@selector(webImageManager:didProgressWithPartialImage:forURL:userInfo:)])
            {
                ...
                NSMutableDictionary *userInfoMutable = nil;
                if ([userInfo isKindOfClass:NSNull.class])
                {
                    userInfo = nil;
                }
                else{
                    userInfoMutable = [userInfo mutableCopy];
                    [userInfoMutable setObject:@(progress) forKey:@"progress"];
                }
                objc_msgSend(delegate, @selector(webImageManager:didProgressWithPartialImage:forURL:userInfo:), self, image, downloader.url, userInfoMutable);
            }
        }
    }
}
