...
- (void)connection:(NSURLConnection *)aConnection didReceiveData:(NSData *)data
{
    ...
    if (self.progressive && expectedSize > 0 && [delegate respondsToSelector:@selector(imageDownloader:didUpdatePartialImage:progress:)])
    {
            ...
            if (partialImageRef)
            {
               ...
               ;
                ...
                [[SDWebImageDecoder sharedImageDecoder] decodeImage:image
                 ...
                 @"progress":@(recievedLength / (float)expectedLength)}];
...
- (void)imageDecoder:(SDWebImageDecoder *)decoder didFinishDecodingImage:(UIImage *)image userInfo:(NSDictionary *)aUserInfo
{
    ...
    if ([[aUserInfo valueForKey:@"type"] isEqualToString:@"partial"])
    {
        ...
        [delegate imageDownloader:self didUpdatePartialImage:image progress:[aUserInfo[@"progress"] doubleValue]];
    }
    else
    {
